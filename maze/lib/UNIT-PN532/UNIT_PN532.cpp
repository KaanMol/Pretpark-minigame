...
/*!
    @file     UNIT_PN532.cpp
          ...
          Driver for NXP's PN532 NFC/13.56MHz RFID Transceiver for ESP.

          This is a library for PN532 NFC/RFID breakout boards
...
*/
/**************************************************************************/
UNIT_PN532::UNIT_PN532(uint8_t clk, uint8_t miso, uint8_t mosi,
                               uint8_t ss) {
...
*/
/**************************************************************************/
UNIT_PN532::UNIT_PN532(uint8_t irq, uint8_t reset)
...
*/
/**************************************************************************/
UNIT_PN532::UNIT_PN532(uint8_t ss) {
  spi_dev =
...
*/
/**************************************************************************/
void UNIT_PN532::PrintHex(const byte *data, const uint32_t numBytes) {
  uint32_t szPos;
...
*/
/**************************************************************************/
void UNIT_PN532::PrintHexChar(const byte *data, const uint32_t numBytes) {
  uint32_t szPos;
  for (szPos = 0; szPos < numBytes; szPos++) {
    // Append leading 0 for small values
    if (data[szPos] <= 0xF)
...
*/
/**************************************************************************/
uint32_t UNIT_PN532::getFirmwareVersion(void) {
  uint32_t response;
...
bool UNIT_PN532::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen,
                                         uint16_t timeout) {
  // uint16_t timer = 0;
...
*/
/**************************************************************************/
bool UNIT_PN532::writeGPIO(uint8_t pinstate) {
  // uint8_t errorbit;
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::readGPIO(void) {
...
*/
/**************************************************************************/
bool UNIT_PN532::SAMConfig(void) {
...
*/
/**************************************************************************/
bool UNIT_PN532::setPassiveActivationRetries(uint8_t maxRetries) {
...
*/
/**************************************************************************/
bool UNIT_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid,
...
*/
/**************************************************************************/
bool UNIT_PN532::startPassiveTargetIDDetection(uint8_t cardbaudrate) {
  pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
  pn532_packetbuffer[1] = 1; // max 1 cards at once (we can set this to 2 later)
  pn532_packetbuffer[2] = cardbaudrate;
...
*/
/**************************************************************************/
bool UNIT_PN532::readDetectedPassiveTargetID(uint8_t *uid,
                                                 uint8_t *uidLength) {
  // read data packet
...
*/
/**************************************************************************/
bool UNIT_PN532::inDataExchange(uint8_t *send, uint8_t sendLength,
                                    uint8_t *response,
...
*/
/**************************************************************************/
bool UNIT_PN532::inListPassiveTarget() {
  pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
...
*/
/**************************************************************************/
bool UNIT_PN532::mifareclassic_IsFirstBlock(uint32_t uiBlock) {
...
*/
/**************************************************************************/
bool UNIT_PN532::mifareclassic_IsTrailerBlock(uint32_t uiBlock) {
  // Test if we are in the small or big sectors
  if (uiBlock < 128)
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::mifareclassic_AuthenticateBlock(uint8_t *uid,
                                                        uint8_t uidLen,
  ...
  if (pn532_packetbuffer[7] != 0x00) {
    ...
    UNIT_PN532::PrintHexChar(pn532_packetbuffer, 12);
#endif
    return 0;
  }
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::mifareclassic_ReadDataBlock(uint8_t blockNumber,
                                                    uint8_t *data) {
#ifdef MIFAREDEBUG
  ...
  if (pn532_packetbuffer[7] != 0x00) {
    ...
    UNIT_PN532::PrintHexChar(pn532_packetbuffer, 26);
#endif
    return 0;
  }
...
/* Display data for debug if requested */
  ...
  PN532DEBUGPRINT.println(blockNumber);
  UNIT_PN532::PrintHexChar(data, 16);
#endif
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::mifareclassic_WriteDataBlock(uint8_t blockNumber,
                                                     uint8_t *data) {
#ifdef MIFAREDEBUG
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::mifareclassic_FormatNDEF(void) {
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::mifareclassic_WriteNDEFURI(uint8_t sectorNumber,
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::mifareultralight_ReadPage(uint8_t page,
                                                  uint8_t *buffer) {
  if (page >= 64) {
#ifdef MIFAREDEBUG
...
/* Display data for debug if requested */
  ...
  PN532DEBUGPRINT.println(F(":"));
  UNIT_PN532::PrintHexChar(buffer, 4);
#endif
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::mifareultralight_WritePage(uint8_t page,
                                                   uint8_t *data) {
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::ntag2xx_ReadPage(uint8_t page, uint8_t *buffer) {
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::ntag2xx_WritePage(uint8_t page, uint8_t *data) {
  // TAG Type       PAGES   USER START    USER STOP
  // --------       -----   ----------    ---------
  // NTAG 203       42      4             39
  // NTAG 213       45      4             39
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::ntag2xx_WriteNDEFURI(uint8_t uriIdentifier, char *url,
                                             uint8_t dataLen) {
...
*/
/**************************************************************************/
bool UNIT_PN532::waitready(uint16_t timeout) {
  uint16_t timer = 0;
  while (!isready()) {
    if (timeout != 0) {
      timer += 10;
...
*/
/**************************************************************************/
void UNIT_PN532::readdata(uint8_t *buff, uint8_t n) {
  if (spi_dev) {
    uint8_t cmd = PN532_SPI_DATAREAD;
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::getDataTarget(uint8_t *cmd, uint8_t *cmdlen) {
  uint8_t length;
...
*/
/**************************************************************************/
uint8_t UNIT_PN532::setDataTarget(uint8_t *cmd, uint8_t cmdlen) {
  uint8_t length;
  // cmd1[0] = 0x8E; Must!
...
*/
/**************************************************************************/
void UNIT_PN532::writecommand(uint8_t *cmd, uint8_t cmdlen) {
  if (spi_dev != NULL) {
    // SPI command write.
    uint8_t checksum;
